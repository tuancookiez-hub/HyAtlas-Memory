// Package main — single-file embedded assets (Linux).
//
// This file is compiled with go:build embedded && linux to bundle the BGE
// model, vocab, and libonnxruntime.so INSIDE the binary. At startup the
// embedded assets are written to a cache dir, then the BGE embedder loads
// them from there. Result: one binary that runs anywhere on Linux with no
// external files.
//
// Build the embedded variant:
//   go build -tags embedded -o hyatlas-go .
// when models/ (with libonnxruntime.so alongside the onnx file) sits next
// to the source. Without the tag, the server reads ./models at runtime.
//go:build embedded && linux
...
//go:embed models/libonnxruntime.so
var embeddedLinuxSO []byte
...
func materializeAssets() string {
	extractOnce.Do(func() {
		...
		files := map[string][]byte{
			...
			"libonnxruntime.so":           embeddedLinuxSO,
		}
