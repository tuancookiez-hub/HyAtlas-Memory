...
> **v4.0.0 just landed — full rewrite to a single Go binary.** 17.6 MB, no Python, no Qdrant sidecar, no zvec LOCK file dance. Same wire contract as v3.5 (the existing `HyMemoryClient` keeps working unchanged), all 7 layers active by default, and retrieval quality jumped from **0.33 → 0.80** with the same BGE-small encoder. **v3.5 is now in maintenance-only mode** — the `master` branch still has the Python code for users who can't run a Go binary, but new installs should use v4.
>
> See [V4 vs v3.5 Comparison Review](./V3_V4_COMPARISON.md) for the full side-by-side review (architecture, performance, reliability, configuration, API compatibility).
...
> **v3.5 (Python) is the legacy stack** — still functional, but maintenance-only. New installs should use v4 (see above). v3.5 consists of: dedicated venv isolation; truthful zvec/Kuzu dashboard contracts; bounded list/count APIs; extraction recovery; Windows lifecycle hardening; and **BGE-small (384d)** as the recommended local embedder. zvec 0.6.0 crash-reopen behavior is regression-tested. Upgrade: `pip install -U git+https://github.com/tuancookiez-hub/HyAtlas-Memory.git`, run `hyatlas venv setup`, then `hyatlas stop && hyatlas start --detach`. See [CHANGELOG](./CHANGELOG.md). The v4 rewrite (single Go binary) lives in a separate branch — see the [v4-rewrite branch](https://github.com/tuancookiez-hub/HyAtlas-Memory/tree/v4-rewrite) for the Go code and the [V4 vs v3.5 Comparison Review](./V3_V4_COMPARISON.md).



<p align="center">

  <img src="./assets/hyatlas-v4.0-banner.png" alt="HyAtlas v4.0: pure Go rewrite, single 17.6 MB binary, Chromem-go + in-process BGE-small via onnxruntime-go, all 7 layers active by default, retrieval 0.80 vs 0.33 (v3.5), wire-compatible with v3.5 HyMemoryClient" width="100%" />
