...
Single command. No PyPI involvement as of v3.4.2 — install is GitHub-direct.
...
Version matrix that works (as of v3.4.2):
- `transformers==4.46.3`
- `huggingface-hub<1.0,>=0.23.2` (not 1.x — that's the bug v3.4.2 fixed)
...
- Check version matrix above. v3.4.2+ should have correct pins.
...
Releases are GitHub-only. No PyPI. Install is `pip install git+https://...` (no version pin → gets latest). To pin a specific version: `pip install git+https://github.com/tuancookiez-hub/HyAtlas-Memory.git@v3.4.2`.
